...
export default async function handler(
  ...
  const rawFulfilledDates = await prisma.$queryRaw<{ date: number }[]>`
    SELECT 
  ...
  const fulfilledDates = rawFulfilledDates.map((item) => item.date)

  return res.json({ blockedWeekDays, fulfilledDates })
}
